Диалог: 
{
	Параметры формы: 
	{
		Параметры: 
		{
			Ширина: 509
			Высота: 290
			Заголовок: " "
			АвтоПорядокОбхода: Нет
			Param24: 6
			Param25: 28
			Слои
			{
				Шапка: 
				Основной: 
				Результат: 
				Подвал: 
				Запрос: 
				Товары: Видимый, Активный
				ТТН: 
			}
		}
	}
	Элементы: 
	{
		1CEDIT: ОсобыеОтметки
		{
			Заголовок: "СпециальныеОтметки"
			ИдентификаторМетаданных: Документ.ВСД_Партия.ОсобыеОтметки
			Тип: Строка
			Длина: 99
			ДопФлаги: Flag23, Flag28, Flag30
			X: 89
			Y: 213
			Ширина: 347
			Высота: 14
			Атрибут: Да
			ПорядокОбхода: 4152
			Слой: Товары
		}
		STATIC: 
		{
			Заголовок: "Особые отметки:"
			ИдентификаторМетаданных: -1
			ДопФлаги: Flag9, Flag28, Flag30
			X: 23
			Y: 213
			Ширина: 64
			Высота: 14
			ПорядокОбхода: 4153
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Товары
		}
		1CEDIT: ДатаСрокГодности
		{
			Заголовок: "ДатаСрокГодности"
			ИдентификаторМетаданных: Документ.ВСД_Партия.ДатаСрокГодности
			Тип: Строка
			Длина: 99
			ДопФлаги: Flag23, Flag28, Flag30
			X: 284
			Y: 122
			Ширина: 114
			Высота: 13
			Атрибут: Да
			ПорядокОбхода: 4154
			Слой: Товары
		}
		STATIC: 
		{
			Заголовок: "Срок годности:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 223
			Y: 122
			Ширина: 60
			Высота: 14
			ПорядокОбхода: 4155
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Товары
		}
		1CEDIT: ВсдМестность
		{
			Заголовок: "ВсдМестность"
			ИдентификаторМетаданных: Документ.ВСД_Партия.ВсдМестность
			Тип: Строка
			Длина: 99
			ДопФлаги: Flag23, Flag28, Flag30
			X: 89
			Y: 196
			Ширина: 347
			Высота: 14
			Атрибут: Да
			ПорядокОбхода: 4156
			Слой: Товары
		}
		STATIC: 
		{
			Заголовок: "Местность:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 23
			Y: 196
			Ширина: 64
			Высота: 14
			ПорядокОбхода: 4157
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Товары
		}
		1CEDIT: ДатаИзготовления
		{
			Заголовок: "ДатаИзготовления"
			ИдентификаторМетаданных: Документ.ВСД_Партия.ДатаИзготовления
			Тип: Строка
			Длина: 99
			ДопФлаги: Flag23, Flag28, Flag30
			X: 86
			Y: 123
			Ширина: 103
			Высота: 13
			Атрибут: Да
			ПорядокОбхода: 4158
			Слой: Товары
		}
		STATIC: 
		{
			Заголовок: "Изготовлено:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 26
			Y: 123
			Ширина: 53
			Высота: 14
			ПорядокОбхода: 4159
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Товары
		}
		1CEDIT: ВсдЭкспертиза
		{
			Заголовок: "ВсдЭкспертиза"
			ИдентификаторМетаданных: Документ.ВСД_Партия.ВсдЭкспертиза
			Тип: Строка
			Длина: 99
			ДопФлаги: Flag23, Flag28, Flag30
			X: 89
			Y: 179
			Ширина: 347
			Высота: 14
			Атрибут: Да
			ПорядокОбхода: 4160
			Слой: Товары
		}
		STATIC: 
		{
			Заголовок: "Экспертиза:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 23
			Y: 179
			Ширина: 64
			Высота: 14
			ПорядокОбхода: 4161
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Товары
		}
		BMASKED: ВсдДата
		{
			Заголовок: "ВсдДата"
			ИдентификаторМетаданных: Документ.ВСД_Партия.ВсдДата
			Тип: Дата
			ДопФлаги: Flag23, Flag28, Flag30
			X: 361
			Y: 160
			Ширина: 70
			Высота: 14
			Атрибут: Да
			ПорядокОбхода: 4162
			ОснФлаги: Педалька
			Слой: Товары
		}
		STATIC: 
		{
			Заголовок: "Дата:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 332
			Y: 160
			Ширина: 23
			Высота: 14
			ПорядокОбхода: 4163
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Товары
		}
		1CEDIT: ВсдНомер
		{
			Заголовок: "ВсдНомер"
			ИдентификаторМетаданных: Документ.ВСД_Партия.ВсдНомер
			Тип: Строка
			Длина: 10
			ДопФлаги: Flag23, Flag28, Flag30
			X: 232
			Y: 160
			Ширина: 77
			Высота: 14
			Атрибут: Да
			ПорядокОбхода: 4164
			Слой: Товары
		}
		STATIC: 
		{
			Заголовок: "№"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 218
			Y: 160
			Ширина: 16
			Высота: 14
			ПорядокОбхода: 4165
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Товары
		}
		1CEDIT: ВсдСерия
		{
			Заголовок: "ВсдСерия"
			ИдентификаторМетаданных: Документ.ВСД_Партия.ВсдСерия
			Тип: Строка
			Длина: 10
			ДопФлаги: Flag23, Flag28, Flag30
			X: 91
			Y: 160
			Ширина: 102
			Высота: 14
			Атрибут: Да
			ПорядокОбхода: 4166
			Слой: Товары
		}
		STATIC: 
		{
			Заголовок: "Серия:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 25
			Y: 160
			Ширина: 64
			Высота: 14
			ПорядокОбхода: 4167
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Товары
		}
		1CGROUPBOX: 
		{
			Заголовок: "Вет свидетельство"
			ДопФлаги: Flag0, Flag1, Flag2, Flag28, Flag30
			X: 19
			Y: 145
			Ширина: 453
			Высота: 91
			ПорядокОбхода: 4168
			Слой: Товары
		}
		1CEDIT: номерАвто
		{
			Заголовок: "номерАвто"
			ИдентификаторМетаданных: Документ.ВСД_Партия.номерАвто
			Тип: Строка
			Длина: 10
			ДопФлаги: Flag23, Flag28, Flag30
			X: 80
			Y: 122
			Ширина: 102
			Высота: 14
			Атрибут: Да
			ПорядокОбхода: 4169
			Слой: ТТН
		}
		STATIC: 
		{
			Заголовок: "номерАвто:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 33
			Y: 122
			Ширина: 42
			Высота: 14
			ПорядокОбхода: 4170
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: ТТН
		}
		BMASKED: КоличествоКор
		{
			Заголовок: "КоличествоКор"
			ИдентификаторМетаданных: Документ.ВСД_Партия.КоличествоКор
			Тип: Число
			Длина: 10
			ДопФлаги: Flag23, Flag28, Flag30
			X: 284
			Y: 105
			Ширина: 48
			Высота: 13
			Атрибут: Да
			ПорядокОбхода: 4171
			ОснФлаги: Педалька
			Слой: Товары
		}
		STATIC: 
		{
			Заголовок: "ТтнДата:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 33
			Y: 105
			Ширина: 42
			Высота: 14
			ПорядокОбхода: 4172
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: ТТН
		}
		BMASKED: ТтнДата
		{
			Заголовок: "ТтнДата"
			ИдентификаторМетаданных: Документ.ВСД_Партия.ТтнДата
			Тип: Дата
			ДопФлаги: Flag23, Flag28, Flag30
			X: 80
			Y: 105
			Ширина: 102
			Высота: 14
			Атрибут: Да
			ПорядокОбхода: 4173
			ОснФлаги: Педалька
			Слой: ТТН
		}
		STATIC: 
		{
			Заголовок: "Кол-во:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 26
			Y: 105
			Ширина: 60
			Высота: 14
			ПорядокОбхода: 4174
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Товары
		}
		BMASKED: Количество
		{
			Заголовок: "Количество"
			ИдентификаторМетаданных: Документ.ВСД_Партия.Количество
			Тип: Число
			Длина: 15
			Точность: 3
			ДопФлаги: Flag23, Flag28, Flag30
			X: 86
			Y: 105
			Ширина: 56
			Высота: 13
			Атрибут: Да
			ПорядокОбхода: 4175
			ОснФлаги: Педалька
			Слой: Товары
		}
		STATIC: 
		{
			Заголовок: "ТтнНомер:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 33
			Y: 88
			Ширина: 42
			Высота: 14
			ПорядокОбхода: 4176
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: ТТН
		}
		1CEDIT: ТтнНомер
		{
			Заголовок: "ТтнНомер"
			ИдентификаторМетаданных: Документ.ВСД_Партия.ТтнНомер
			Тип: Строка
			Длина: 10
			ДопФлаги: Flag23, Flag28, Flag30
			X: 80
			Y: 88
			Ширина: 102
			Высота: 14
			Атрибут: Да
			ПорядокОбхода: 4177
			Слой: ТТН
		}
		BMASKED: ВидПродукции
		{
			Заголовок: "ВидПродукции"
			ИдентификаторМетаданных: Документ.ВСД_Партия.ВидПродукции
			Тип: Справочник
			Агрегатный тип: Справочник.ВСД_ВидПродукции
			ДопФлаги: Flag23, Flag28, Flag30
			X: 86
			Y: 88
			Ширина: 362
			Высота: 13
			Атрибут: Да
			ПорядокОбхода: 4178
			ОснФлаги: Педалька
			Слой: Товары
		}
		STATIC: 
		{
			Заголовок: "Продукция:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 26
			Y: 53
			Ширина: 60
			Высота: 14
			ПорядокОбхода: 4179
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Товары
		}
		BMASKED: Продукция
		{
			Заголовок: "Продукция"
			ИдентификаторМетаданных: Документ.ВСД_Партия.Продукция
			Тип: Справочник
			Агрегатный тип: Справочник.ВСД_Продукция
			ДопФлаги: Flag23, Flag28, Flag30
			X: 86
			Y: 54
			Ширина: 362
			Высота: 13
			Атрибут: Да
			ПорядокОбхода: 4180
			ОснФлаги: Педалька
			Слой: Товары
		}
		STATIC: 
		{
			Заголовок: "ТтнСерия:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 33
			Y: 71
			Ширина: 42
			Высота: 14
			ПорядокОбхода: 4181
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: ТТН
		}
		1CEDIT: ТтнСерия
		{
			Заголовок: "ТтнСерия"
			ИдентификаторМетаданных: Документ.ВСД_Партия.ТтнСерия
			Тип: Строка
			Длина: 10
			ДопФлаги: Flag23, Flag28, Flag30
			X: 80
			Y: 71
			Ширина: 102
			Высота: 14
			Атрибут: Да
			ПорядокОбхода: 4182
			Слой: ТТН
		}
		1CGROUPBOX: 
		{
			Заголовок: "ТТН"
			ДопФлаги: Flag0, Flag1, Flag2, Flag28, Flag30
			X: 24
			Y: 60
			Ширина: 172
			Высота: 85
			ПорядокОбхода: 4183
			Слой: ТТН
		}
		STATIC: 
		{
			Заголовок: "ВидПродукции:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 26
			Y: 88
			Ширина: 60
			Высота: 14
			ПорядокОбхода: 4184
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Товары
		}
		1CGROUPBOX: 
		{
			Заголовок: "Продукция"
			ДопФлаги: Flag0, Flag1, Flag2, Flag28, Flag30
			X: 19
			Y: 43
			Ширина: 453
			Высота: 97
			ПорядокОбхода: 4185
			Слой: Товары
		}
		1CEDIT: RequestResponse
		{
			Заголовок: "Ответ на запрос"
			ИдентификаторМетаданных: Документ.ВСД_Партия.RequestResponse
			Тип: Строка
			ДопФлаги: Flag23, Flag28, Flag30
			X: 10
			Y: 158
			Ширина: 445
			Высота: 54
			Атрибут: Да
			ПорядокОбхода: 4186
			ОснФлаги: Многострочное
			Слой: Запрос
		}
		STATIC: 
		{
			Заголовок: "RequestResponse"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 14
			Y: 143
			Ширина: 63
			Высота: 13
			ПорядокОбхода: 4187
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Запрос
		}
		1CEDIT: Request
		{
			Заголовок: "Request"
			ИдентификаторМетаданных: Документ.ВСД_Партия.Request
			Тип: Строка
			ДопФлаги: Flag23, Flag28, Flag30
			X: 8
			Y: 52
			Ширина: 448
			Высота: 85
			Атрибут: Да
			ПорядокОбхода: 4188
			ОснФлаги: Многострочное
			Слой: Запрос
		}
		STATIC: 
		{
			Заголовок: "Request:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 13
			Y: 35
			Ширина: 87
			Высота: 13
			ПорядокОбхода: 4189
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Запрос
		}
		STATIC: 
		{
			ИдентификаторМетаданных: -1
			ДопФлаги: Flag9, Flag28, Flag30
			X: 250
			Y: 269
			Ширина: 195
			Высота: 13
			ПорядокОбхода: 4190
			Формула: "Автор"
			ОснФлаги: ШрифтПоУмолчанию, ПрозрачныйФон
			Hint: "Автор документа (данные \nиз справочника ""Пользователи"")"
			ShortHint: "Автор документа (пользователь)"
			ЦветТекста: 128
			Слой: Подвал
		}
		BUTTON: кнДействия
		{
			Заголовок: "&Действия..."
			ИдентификаторМетаданных: -1
			ДопФлаги: Flag0, Flag1, КартинкаПоЦентру, Flag28, Flag30
			X: 185
			Y: 268
			Ширина: 54
			Высота: 13
			ПорядокОбхода: 4191
			Формула: "глДействия(Контекст,СписокДействий)"
			ОснФлаги: ПропускатьПриВводе
			Hint: "Нажатие этой кнопки \nвызывает меню со списком действий, \nкоторые могут быть выполнены с\nдокументом. Подробная информация \nо списке действий по документу \nприведена в его описании"
			ShortHint: "Действия по документу"
			Слой: Подвал
		}
		BUTTON: кнПечать
		{
			Заголовок: "&Печать"
			ИдентификаторМетаданных: -1
			ДопФлаги: Flag0, Flag1, КартинкаПоЦентру, Flag28, Flag30
			X: 139
			Y: 268
			Ширина: 43
			Высота: 13
			ПорядокОбхода: 4192
			Формула: "ПоКнопкеПечать()"
			ОснФлаги: ИспользоватьОписание, КартинкаПоЦентру
			Hint: "Сформировать печатную форму \nдокумента"
			ShortHint: "Печать накладной"
			Слой: Подвал
			HotKey: 3+F8
		}
		BUTTON: кнВыбПечать
		{
			ИдентификаторМетаданных: -1
			ДопФлаги: Flag0, Flag1, КартинкаПоЦентру, Flag28, Flag30
			X: 128
			Y: 268
			Ширина: 11
			Высота: 13
			ПорядокОбхода: 4193
			Формула: "ПоКнопкеВыборПечатнойФормы()"
			ОснФлаги: ПропускатьПриВводе, КартинкаПропорционально
			Hint: "Выбор печатной формы \nдокумента. Выбранная форма \nсохраняется на следующий сеанс \nработы с документом"
			ShortHint: "Выбрать печатную форму"
			Пиктограмма: 0x1000000e
			Слой: Подвал
		}
		BUTTON: 
		{
			Заголовок: "Закрыть"
			ИдентификаторМетаданных: -1
			ДопФлаги: Flag0, Flag1, КартинкаПоЦентру, Flag28, Flag30
			X: 70
			Y: 268
			Ширина: 54
			Высота: 13
			ПорядокОбхода: 4194
			Формула: "#Закрыть"
			Слой: Подвал
		}
		BUTTON: 
		{
			Заголовок: "ОК"
			ИдентификаторМетаданных: -1
			ДопФлаги: Flag0, Flag1, КартинкаПоЦентру, КнопкаПоУмолчанию, Flag28, Flag30
			X: 13
			Y: 268
			Ширина: 54
			Высота: 13
			ПорядокОбхода: 4195
			Формула: "#Записать Закрыть"
			Слой: Подвал
		}
		1CEDIT: Комментарий
		{
			Заголовок: "Комментарий"
			ИдентификаторМетаданных: 660
			Тип: Строка
			Длина: 200
			ДопФлаги: Flag23, Flag28, Flag30
			X: 70
			Y: 247
			Ширина: 336
			Высота: 14
			Атрибут: Да
			ПорядокОбхода: 4196
			Слой: Подвал
		}
		STATIC: 
		{
			Заголовок: "Комментарий:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 12
			Y: 246
			Ширина: 50
			Высота: 14
			ПорядокОбхода: 4197
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Подвал
		}
		1CEDIT: RequestResult
		{
			Заголовок: "RequestResult"
			ИдентификаторМетаданных: Документ.ВСД_Партия.RequestResult
			Тип: Строка
			ДопФлаги: Flag23, Flag28, Flag30
			X: 7
			Y: 50
			Ширина: 450
			Высота: 123
			Атрибут: Да
			ПорядокОбхода: 4198
			ОснФлаги: Многострочное
			Слой: Результат
		}
		STATIC: 
		{
			Заголовок: "Ответ на запрос:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 11
			Y: 33
			Ширина: 68
			Высота: 13
			ПорядокОбхода: 4199
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Результат
		}
		BMASKED: Производитель_Площадка
		{
			Заголовок: "Производитель_ХозСубъект"
			ИдентификаторМетаданных: Документ.ВСД_Партия.Производитель_Площадка
			Тип: Справочник
			Агрегатный тип: Справочник.ВСД_Площадка
			ДопФлаги: Flag23, Flag28, Flag30
			X: 73
			Y: 168
			Ширина: 154
			Высота: 14
			Атрибут: Да
			ПорядокОбхода: 4200
			ОснФлаги: Педалька
			Маска: Производитель_ХозСубъект
			Слой: Основной
		}
		STATIC: 
		{
			Заголовок: "Площадка"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 15
			Y: 169
			Ширина: 41
			Высота: 14
			ПорядокОбхода: 4201
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Основной
		}
		BMASKED: Производитель_ХозСубъект
		{
			Заголовок: "Производитель_ХозСубъект"
			ИдентификаторМетаданных: Документ.ВСД_Партия.Производитель_ХозСубъект
			Тип: Справочник
			Агрегатный тип: Справочник.ВСД_ХозСубъект
			ДопФлаги: Flag23, Flag28, Flag30
			X: 73
			Y: 150
			Ширина: 154
			Высота: 14
			Атрибут: Да
			ПорядокОбхода: 4202
			ОснФлаги: Педалька
			Слой: Основной
		}
		STATIC: 
		{
			Заголовок: "ХозСубъект:"
			ИдентификаторМетаданных: -1
			ДопФлаги: Flag9, Flag28, Flag30
			X: 15
			Y: 150
			Ширина: 47
			Высота: 14
			ПорядокОбхода: 4203
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Основной
		}
		1CGROUPBOX: 
		{
			Заголовок: "Производитель"
			ИдентификаторМетаданных: -1
			ДопФлаги: Flag0, Flag1, Flag2, Flag28, Flag30
			X: 11
			Y: 139
			Ширина: 447
			Высота: 54
			ПорядокОбхода: 4204
			Слой: Основной
		}
		BMASKED: Получатель_Площадка
		{
			Заголовок: "Получатель_Площадка"
			ИдентификаторМетаданных: Документ.ВСД_Партия.Получатель_Площадка
			Тип: Справочник
			Агрегатный тип: Справочник.ВСД_Площадка
			ДопФлаги: Flag23, Flag28, Flag30
			X: 73
			Y: 106
			Ширина: 154
			Высота: 14
			Атрибут: Да
			ПорядокОбхода: 4205
			ОснФлаги: Педалька
			Маска: Получатель_ХозСубъект
			Слой: Основной
		}
		STATIC: 
		{
			Заголовок: "Площадка:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 15
			Y: 106
			Ширина: 41
			Высота: 14
			ПорядокОбхода: 4206
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Основной
		}
		BMASKED: Получатель_ХозСубъект
		{
			Заголовок: "Получатель_ХозСубъект"
			ИдентификаторМетаданных: Документ.ВСД_Партия.Получатель_ХозСубъект
			Тип: Справочник
			Агрегатный тип: Справочник.ВСД_ХозСубъект
			ДопФлаги: Flag23, Flag28, Flag30
			X: 73
			Y: 89
			Ширина: 154
			Высота: 14
			Атрибут: Да
			ПорядокОбхода: 4207
			ОснФлаги: Педалька
			Слой: Основной
		}
		STATIC: 
		{
			Заголовок: "ХозСубъект:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 15
			Y: 89
			Ширина: 47
			Высота: 14
			ПорядокОбхода: 4208
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Основной
		}
		1CGROUPBOX: 
		{
			Заголовок: "Получатель"
			ИдентификаторМетаданных: -1
			ДопФлаги: Flag0, Flag1, Flag2, Flag28, Flag30
			X: 11
			Y: 77
			Ширина: 447
			Высота: 61
			ПорядокОбхода: 4209
			Слой: Основной
		}
		STATIC: 
		{
			Заголовок: "Статус:"
			ДопФлаги: Flag28, Flag30
			X: 196
			Y: 19
			Ширина: 33
			Высота: 11
			ПорядокОбхода: 4210
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Шапка
		}
		BMASKED: ДатаДок
		{
			Заголовок: "ДатаДок"
			ИдентификаторМетаданных: -2571
			Тип: Дата
			ДопФлаги: Flag23, Flag28, Flag30
			X: 119
			Y: 18
			Ширина: 62
			Высота: 13
			Атрибут: Да
			ПорядокОбхода: 4211
			ОснФлаги: Педалька
			Слой: Шапка
		}
		STATIC: 
		{
			Заголовок: "от"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 102
			Y: 18
			Ширина: 13
			Высота: 14
			ПорядокОбхода: 4212
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Шапка
		}
		1CEDIT: НомерДок
		{
			Заголовок: "НомерДок"
			ИдентификаторМетаданных: -2570
			Тип: Число
			Длина: 5
			ДопФлаги: Flag23, Flag28, Flag30
			X: 37
			Y: 18
			Ширина: 60
			Высота: 13
			Атрибут: Да
			ПорядокОбхода: 4213
			Слой: Шапка
		}
		STATIC: 
		{
			Заголовок: "№"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 16
			Y: 18
			Ширина: 13
			Высота: 14
			ПорядокОбхода: 4214
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Шапка
		}
		STATIC: 
		{
			Заголовок: "Наименование:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 26
			Y: 70
			Ширина: 60
			Высота: 14
			ПорядокОбхода: 4215
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Товары
		}
		1CEDIT: НаименованиеПродукции
		{
			Заголовок: "НаименованиеПродукции"
			ИдентификаторМетаданных: Документ.ВСД_Партия.НаименованиеПродукции
			Тип: Строка
			Длина: 300
			ДопФлаги: Flag23, Flag28, Flag30
			X: 86
			Y: 70
			Ширина: 362
			Высота: 13
			Атрибут: Да
			ПорядокОбхода: 4216
			Слой: Товары
		}
		STATIC: 
		{
			Заголовок: "GUID:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 12
			Y: 39
			Ширина: 23
			Высота: 13
			ПорядокОбхода: 4217
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Основной
		}
		1CEDIT: GUID
		{
			Заголовок: "GUID"
			ИдентификаторМетаданных: Документ.ВСД_Партия.GUID
			Тип: Строка
			Длина: 99
			ДопФлаги: Flag23, Flag28, Flag30
			X: 38
			Y: 40
			Ширина: 207
			Высота: 13
			Атрибут: Да
			ПорядокОбхода: 4218
			Слой: Основной
		}
		STATIC: 
		{
			Заголовок: "UUID:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 12
			Y: 57
			Ширина: 23
			Высота: 13
			ПорядокОбхода: 4219
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Основной
		}
		1CEDIT: UUID
		{
			Заголовок: "UUID"
			ИдентификаторМетаданных: Документ.ВСД_Партия.UUID
			Тип: Строка
			Длина: 99
			ДопФлаги: Flag23, Flag28, Flag30
			X: 38
			Y: 57
			Ширина: 207
			Высота: 13
			Атрибут: Да
			ПорядокОбхода: 4220
			Слой: Основной
		}
		STATIC: 
		{
			Заголовок: "Дата Изменения:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 323
			Y: 40
			Ширина: 60
			Высота: 14
			ПорядокОбхода: 4221
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Основной
		}
		BMASKED: ДатаИзменения
		{
			Заголовок: "ДатаИзменения"
			ИдентификаторМетаданных: Документ.ВСД_Партия.ДатаИзменения
			Тип: Дата
			ДопФлаги: Flag23, Flag28, Flag30
			X: 389
			Y: 40
			Ширина: 107
			Высота: 13
			Атрибут: Да
			ПорядокОбхода: 4222
			ОснФлаги: Педалька
			Слой: Основной
		}
		STATIC: 
		{
			Заголовок: "Номер записи:"
			ДопФлаги: Flag9, Flag28, Flag30
			X: 323
			Y: 57
			Ширина: 60
			Высота: 14
			ПорядокОбхода: 4223
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Основной
		}
		1CEDIT: НомерЗаписи
		{
			Заголовок: "НомерЗаписи"
			ИдентификаторМетаданных: Документ.ВСД_Партия.НомерЗаписи
			Тип: Строка
			Длина: 99
			ДопФлаги: Flag23, Flag28, Flag30
			X: 389
			Y: 57
			Ширина: 107
			Высота: 13
			Атрибут: Да
			ПорядокОбхода: 4224
			Слой: Основной
		}
		STATIC: 
		{
			ИдентификаторМетаданных: -1
			ДопФлаги: Flag28, Flag30
			X: 235
			Y: 18
			Ширина: 102
			Высота: 14
			ПорядокОбхода: 4225
			Формула: "Статус"
			Шрифт: MS Sans Serif
			{
				Жирный: Да
				Param9: 204
				Param10: 1
				Param11: 2
				Param12: 1
				Param13: 34
			}
			ЦветТекста: 16711680
			Слой: Шапка
		}
		BMASKED: ФормаУпаковки
		{
			Заголовок: "ФормаУпаковки"
			ИдентификаторМетаданных: Документ.ВСД_Партия.ФормаУпаковки
			Тип: Справочник
			Агрегатный тип: Справочник.ВСД_ФормыУпаковки
			ДопФлаги: Flag23, Flag28, Flag30
			X: 335
			Y: 105
			Ширина: 63
			Высота: 13
			Атрибут: Да
			ПорядокОбхода: 4226
			ОснФлаги: Педалька
			Слой: Товары
		}
		BMASKED: ЕдиницаИзмерения
		{
			Заголовок: "ЕдиницаИзмерения"
			ИдентификаторМетаданных: Документ.ВСД_Партия.ЕдиницаИзмерения
			Тип: Справочник
			Агрегатный тип: Справочник.ВСД_ЕдиницыИзмерения
			ДопФлаги: Flag23, Flag28, Flag30
			X: 145
			Y: 105
			Ширина: 45
			Высота: 13
			Атрибут: Да
			ПорядокОбхода: 4227
			ОснФлаги: Педалька
			Слой: Товары
		}
		STATIC: 
		{
			Заголовок: "Коробов:"
			ДопФлаги: Flag28, Flag30
			X: 223
			Y: 108
			Ширина: 39
			Высота: 13
			ПорядокОбхода: 4228
			ОснФлаги: ШрифтПоУмолчанию, ЦветТекстаАвто
			Слой: Товары
		}
	}
}
